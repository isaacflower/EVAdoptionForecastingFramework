...
class LADVehicleRegistrationDataProcessor:
    ...
    def process_data(self, query: str, lad_list: np.ndarray, first_num_col: int = 3):
        """
        Preprocess, filter, and transform raw data to LAD-level time series.

        Parameters:
        - query (str): Query string to filter vehicle types (e.g., 'Fuel == "Petrol"').
        ...
        df_ev = self._transform_to_timeseries(self.df_veh0142, query, first_num_col)
        df_v = self._transform_to_timeseries(self.df_veh0105, query, first_num_col)
